<?php


    require 'connection.php';

    // include('connetion.php');

    $a = 15;

    echo "<br>" .$a;



    if(isset($_POST['btn'])){

        $prodName =  $_POST['prodName'];
        $prodPrice = $_POST['prodPrice'];
        $prodDesc = $_POST['prodDesc'];

        echo '<br>' .$prodName. '<br>' .$prodPrice. '<br>' .$prodDesc;


        $insertQuery = "INSERT INTO products (prod_name, prod_price, prod_desc) VALUES ('$prodName','$prodPrice','$prodDesc')";
        $result = mysqli_query($connection, $insertQuery);

        if($result){
            echo "<br> Data insert Succesfully";
        }
        else{
            echo "<br> Data insert failed";

        }



        








        


    }










?>




<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Bootstrap demo</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
   
  </head>
  <body>
    <h1 class="text-center mt-2">ADD PRODUCTS!</h1>

    <form action="" method="post" >
        
        Prdouct Name: <input type="text" name="prodName"> <br>
        Prdouct Price: <input type="text" name="prodPrice"  class="mt-3"> <br>
        Prdouct Description: <input type="text" name="prodDesc"  class="mt-3"> <br> 
        <input type="submit" value="submit" name="btn" class="mt-2" >
        
    </form>




    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
  </body>
</html>